...
// Validación: telefono (paso 2) solo números, obligatorio, máximo 15 dígitos, feedback visual
...
if (telefonoEmpresa) {
	...
	telefonoEmpresa.addEventListener('blur', function() {
		let errorDiv = document.getElementById('telefono-error');
		if (!errorDiv) {
			errorDiv = document.createElement('div');
			errorDiv.id = 'telefono-error';
...
// === FEEDBACKS VISUALES Y MICROINTERACCIONES ===

// Función para mostrar tooltips dinámicos
function showTooltip(element, message, type = 'info') {
	const tooltip = document.createElement('div');
	tooltip.className = `tooltip-content ${type}`;
...
// Función para agregar efecto de carga a botones
function addLoadingState(button, text = 'Procesando...') {
...
function animateOnShow(element, animationType = 'slideInRight') {
	element.style.animation = `${animationType} 0.5s ease`;
}

// Función para agregar feedback visual al progreso del wizard
function updateWizardProgress(currentStep, totalSteps = 5) {
	...
	if (progressBar) {
		...
		// Agregar pulso al progreso
		progressBar.style.animation = 'progressGlow 1s ease-in-out';
		setTimeout(() => {
...
// Función para feedback visual en validaciones
function showFieldFeedback(field, isValid, message = '') {
	...
	if (message) {
		...
		feedback.className = `field-feedback ${isValid ? 'text-success' : 'text-danger'} mt-1`;
		feedback.innerHTML = `<i class="bi ${isValid ? 'bi-check-circle' : 'bi-exclamation-circle'}"></i> ${message}`;
	...
	if (isValid) {
		...
		field.style.animation = 'slideInRight 0.4s ease';
	} else {
...
function showToast(message, type = 'success', duration = 3000) {
	...
	toast.style.cssText = `
		...
		top: 20px;
		right: 20px;
		background: ${type === 'success' ? 'var(--success-green)' : '#dc3545'};
		color: white;
		padding: 12px 16px;
		border-radius: 8px;
...
// Función para manejar transiciones suaves entre pasos
function smoothStepTransition(hideElement, showElement, callback = null) {
	// Efecto de salida
	hideElement.style.animation = 'fadeOut 0.3s ease';
...
document.addEventListener('DOMContentLoaded', function() {
	// === INICIALIZACIÓN DE FEEDBACKS VISUALES ===
	
	// Agregar efectos hover a todos los campos de entrada
	...
	// --- FUNCIONALIDAD DE REGISTRO COMPLETADO ---
	// Solo mostrar mensaje de éxito tras completar el registro
	const userData = {
		nombre: '',
		empresa: '',
		cuit: '',
...
document.addEventListener('DOMContentLoaded', function() {
	...
	if (btnSiguiente1) {
		btnSiguiente1.addEventListener('click', function() {
			...
			const removeLoading = addLoadingState(this, 'Validando...');
			...
			setTimeout(() => {
				...
				// Mostrar feedback visual para CUIT
				showFieldFeedback(cuitInput, !hasErrors, hasErrors ? errorMsg : '¡CUIT válido!');
				
				// Validar Razón Social obligatorio
				let razonErrorMsg = '';
				let razonHasErrors = false;
				if (!razonValue) {
					razonErrorMsg = 'El campo Razón Social es obligatorio.';
					razonHasErrors = true;
				}
				
				// Mostrar feedback visual para Razón Social
				showFieldFeedback(razonInput, !razonHasErrors, razonHasErrors ? razonErrorMsg : '¡Razón social válida!');
				
				removeLoading();
				
				if (!hasErrors && !razonHasErrors) {
					...
					if (hasErrors) cuitInput.focus();
					else if (razonHasErrors) razonInput.focus();
				}
			}, 800); // Simular tiempo de validación
		});
	}
	// Validación duplicada removida - solo mantener la nueva implementación
	// Limpiar el código duplicado que estaba después
	...
	// Código anterior de validación del paso 2 removido por limpieza
			...
			} else if (!/^\d{1,15}$/.test(telefonoValue)) {
				telefonoErrorMsg = 'El teléfono debe tener hasta 15 dígitos.';
			}
			if (telefonoErrorMsg) {
